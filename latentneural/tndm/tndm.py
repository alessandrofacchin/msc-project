...
class TNDM(tf.keras.Model):
  ...
  def __init__(self, **kwargs: Dict[str, Any]):
    ...
    self.tracker_loss_neural_loglike = tf.keras.metrics.Sum(name="loss_neural_loglike")
    self.tracker_loss_behavioural_loglike = tf.keras.metrics.Sum(name="loss_behavioural_loglike")
    self.tracker_loss_relevant_kldiv = tf.keras.metrics.Sum(name="loss_relevant_kldiv")
    ...
    forward_layer = tf.keras.layers.LSTM(self.encoded_space, time_major=False, name="EncoderLSTMForward", return_sequences=True, **encoder_args)
    backward_layer = tf.keras.layers.LSTM(self.encoded_space, time_major=False, name="EncoderLSTMBackward", return_sequences=True, go_backwards=True, **encoder_args)
    ...
    # DISTRIBUTION
    # Relevant
    relevant_dense_mean_args: Dict[str, Any] = ArgsParser.get_or_default(kwargs, 'relevant_dense_mean', {})
    self.relevant_dense_mean = tf.keras.layers.Dense(self.encoded_space, name="RelevantDenseMean", **relevant_dense_mean_args)
    relevant_dense_logvar_args: Dict[str, Any] = ArgsParser.get_or_default(kwargs, 'relevant_dense_logvar', {})
    self.relevant_dense_logvar = tf.keras.layers.Dense(self.encoded_space, name="RelevantDenseLogVar", **relevant_dense_logvar_args)
    # Irrelevant
    irrelevant_dense_mean_args: Dict[str, Any] = ArgsParser.get_or_default(kwargs, 'irrelevant_dense_mean', {})
    self.irrelevant_dense_mean = tf.keras.layers.Dense(self.encoded_space, name="IrrelevantDenseMean", **irrelevant_dense_mean_args)
    irrelevant_dense_logvar_args: Dict[str, Any] = ArgsParser.get_or_default(kwargs, 'irrelevant_dense_logvar', {})
    self.irrelevant_dense_logvar = tf.keras.layers.Dense(self.encoded_space, name="IrrelevantDenseLogVar", **irrelevant_dense_logvar_args)

    # SAMPLING
    self.relevant_sampling = GaussianSampling(name="RelevantGaussianSampling")
    self.irrelevant_sampling = GaussianSampling(name="IrrelevantGaussianSampling")
    ...
    if self.original_generator:
      relevant_decoder_cell = GeneratorGRU(self.encoded_space, **relevant_decoder_args)
      self.relevant_decoder = tf.keras.layers.RNN(relevant_decoder_cell, return_sequences=True, time_major=False, name='RelevantDecoderGRU')
    else:
      self.relevant_decoder = tf.keras.layers.GRU(self.encoded_space, return_sequences=True, time_major=False, name='RelevantDecoderGRU', **relevant_decoder_args)
    # Irrelevant
    ...
    if self.original_generator:
      irrelevant_decoder_cell = GeneratorGRU(self.encoded_space, **irrelevant_decoder_args)
      self.irrelevant_decoder = tf.keras.layers.RNN(irrelevant_decoder_cell, return_sequences=True, time_major=False, name='IrrelevantDecoderGRU')
    else:
      self.irrelevant_decoder = tf.keras.layers.GRU(self.encoded_space, return_sequences=True, time_major=False, name='IrrelevantDecoderGRU', **irrelevant_decoder_args)

    # DIMENSIONALITY REDUCTION
    relevant_factors_dense_args: Dict[str, Any] = ArgsParser.get_or_default(kwargs, 'relevant_factors_dense', {})
    self.relevant_factors_dense = tf.keras.layers.Dense(self.relevant_factors, name="RelevantFactorsDense", activation='tanh', **relevant_factors_dense_args)
    irrelevant_factors_dense_args: Dict[str, Any] = ArgsParser.get_or_default(kwargs, 'irrelevant_factors_dense', {})
    self.irrelevant_factors_dense = tf.keras.layers.Dense(self.irrelevant_factors, name="IrrelevantFactorsDense", activation='tanh', **irrelevant_factors_dense_args)
  ...
  def call(self, inputs, training: bool=True):
    (g0_r, mean_r, logvar_r), (g0_i, mean_i, logvar_i) = self.encode(inputs, training=training)
    log_f, b, (z_r, z_i) = self.decode(g0_r, g0_i, inputs, training=training)
  ...
  def decode(self, g0_r, g0_i, neural, training: bool=True):
    ...
    u_r = tf.stack([tf.zeros_like(neural)[:,:,-1] for i in range(self.relevant_decoder.cell.units)], axis=-1)
    u_i = tf.stack([tf.zeros_like(neural)[:,:,-1] for i in range(self.irrelevant_decoder.cell.units)], axis=-1)
    
    # Relevant
    g0_r_activated = self.relevant_pre_decoder_activation(g0_r)
    g_r = self.relevant_decoder(u_r, initial_state=g0_r_activated, training=training)
    z_r = self.relevant_factors_dense(g_r, training=training)

    # Irrelevant
    g0_i_activated = self.irrelevant_pre_decoder_activation(g0_i)
    g_i = self.irrelevant_decoder(u_i, initial_state=g0_i_activated, training=training)
    ...
    log_f = tf.math.log(self.timestep) + tf.tanh(self.neural_dense(z, training=training)) * 10 # soft-clipping the log-firingrate log(self.timestep) so that the log-likelihood does not return NaN (https://github.com/tensorflow/tensorflow/issues/47019)

    # In order to be able to auto-encode, the dimensions should be the same
    if not self.built:
      assert all([f_i == i_i for f_i, i_i in zip(list(log_f.shape), list(neural.shape))])
  ...
  def encode(self, neural, training: bool=True):
    ...
    if self.encoded_var_trainable:
      logit_var_r = tf.exp(self.relevant_dense_logvar(encoded_reduced, training=training))
      var_r = tf.nn.sigmoid(logit_var_r) * (self.encoded_var_max - self.encoded_var_min) + self.encoded_var_min
      logvar_r = tf.math.log(var_r)
    else:
      logvar_r = tf.zeros_like(mean_r) + self.encoded_var_min
    g0_r = self.relevant_sampling(tf.stack([mean_r, logvar_r], axis=-1), training=training)

    # Irrelevant
    mean_i = self.irrelevant_dense_mean(encoded_reduced, training=training)
    if self.encoded_var_trainable:
      logit_var_i = tf.exp(self.irrelevant_dense_logvar(encoded_reduced, training=training))
      var_i = tf.nn.sigmoid(logit_var_i) * (self.encoded_var_max - self.encoded_var_min) + self.encoded_var_min
      logvar_i = tf.math.log(var_i)
    else:
      logvar_i = tf.zeros_like(mean_i) + self.encoded_var_min
    g0_i = self.irrelevant_sampling(tf.stack([mean_i, logvar_i], axis=-1), training=training)
  ...
  def poisson_loglike_loss(timestep):
    ...
    def loss_fun(y_true, y_pred):
      ...
      log_f = tf.cast(tf.math.log(timestep) + log_f, tf.float32) # Timestep
  ...
  def relevant_kldiv_loss(prior_variance):
    ...
    def loss_fun(y_true, y_pred):
      ...
      dist_prior = tfp.distributions.Normal(0., tf.sqrt(prior_variance), name='RelevantPriorNormal') # PriorVariance
      dist_posterior = tfp.distributions.Normal(mean_r, tf.exp(0.5 * logvar_r), name='RelevantPosteriorNormal')
  ...
  def irrelevant_kldiv_loss(prior_variance):
    ...
    def loss_fun(y_true, y_pred):
      ...
      dist_prior = tfp.distributions.Normal(0., tf.sqrt(prior_variance), name='IrrelevantPriorNormal') # PriorVariance
      dist_posterior = tfp.distributions.Normal(mean_i, tf.exp(0.5 * logvar_i), name='IrrelevantPosteriorNormal')
      return tf.reduce_sum(tfp.distributions.kl_divergence(
        dist_prior, dist_posterior, allow_nan_stats=False, name=None
      ))
    return loss_fun
  ...
  def train_step(self, data):
    ...
    value_clipped_grads = [tf.clip_by_value(x, -1e16, 1e16) if x is not None else x for x in unclipped_grads] # For numerical stability (clip_by_global_norm returns NaNs for large grads, becaues grad_global_norms goes to Inf)
    grads, grad_global_norm = tf.clip_by_global_norm(value_clipped_grads, self.max_grad_norm)
    # Run backwards pass.
    ...
    for grad, var in zip(grads, self.trainable_variables):
      if 'bias' not in var.name.lower():
        ...
        self.tracker_gradient_dict['grads/' + cleaned_name].update_state(tf.norm(grad, 1))
        self.tracker_norms_dict['norms/' + cleaned_name].update_state(tf.norm(var, 1))
