...
class AdaptiveWeights(tf.keras.callbacks.Callback):
  def __init__(
    self,
    initial: List[tf.Variable]=[1., 1.],
    min_weight: List[float]=[0., 0.], 
    max_weight: List[float]=[1., 1.], 
    update_steps: List[int]=[1, 1], 
    update_rates: List[float]=[0., 0.], 
    update_starts: List[int]=[0, 0]):
    """
    ...
    """
    self.w = tf.Variable(initial, shape=tf.TensorShape(2), trainable=False)
  ...
  def on_train_batch_begin(self, batch, logs=None):
    ...
    tf.print('Weight1', self.w[0], 'Weight2', self.w[1], output_stream=sys.stdout)
    self.batches += 1
